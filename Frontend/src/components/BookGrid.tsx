...
const BookGrid: React.FC<BookGridProps> = ({
  ...
  return <div className={`grid ${colClass} gap-4 sm:gap-6 pl-8`}>
    {books.map(book => <div key={book.id} className="flex flex-col group hover:shadow-lg transition duration-300 rounded-md overflow-hidden bg-white">
      <div className="relative pb-[150%] mb-2 overflow-hidden cursor-pointer" onClick={() => handleBookClick(book.id)}>
      ...
      <div className="p-2 flex flex-col flex-grow">
        <h3 className="font-semibold text-sm line-clamp-2 cursor-pointer hover:text-blue-800" onClick={() => handleBookClick(book.id)}>
          {book.title}
        </h3>
